        ...
        {MATCHES.map((m) => (
          ...
          >
            <div className="flex min-w-0 items-center gap-2 font-mono text-[10px] uppercase tracking-[0.25em] text-[#A0A0A0] md:col-span-3">
              {m.status === "live" && <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[#C6FF3D]" />}
              <span className="truncate">{m.stage} · {m.kickoff}</span>
            </div>
            <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 md:col-span-6 md:gap-4">
              ...
              <span className="font-display text-xl text-white sm:text-2xl md:text-3xl">{m.home}</span>
              <span className="font-mono text-xs text-[#A0A0A0]">{m.score}</span>
              <span className="font-display text-xl text-white sm:text-2xl md:text-3xl">{m.away}</span>
              <span className="text-xl sm:text-2xl">{m.awayFlag}</span>
              {m.status === "live" && (
                <span className="rounded-full bg-[#C6FF3D]/10 px-2 py-0.5 font-mono text-[10px] uppercase tracking-widest text-[#C6FF3D]">{m.minute}</span>
              )}
            </div>
